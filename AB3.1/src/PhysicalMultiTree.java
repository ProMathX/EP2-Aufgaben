 ...
 *
 */
public class PhysicalMultiTree {
    ...
    public PhysicalMultiTree() {
        ...
        this.root = new PhysicalMultiTreeNodeEmpty();
        this.size = 0;
    }
    ...
    public Physical[] childrenOf(Physical parent) {
        ...
        PhysicalMultiTreeNode node = root.findNode(parent);
        if (node.getValue() == null) {
            return null;
        }
        PhysicalMultiTreeNode[] childNodes = node.getChildren();
        Physical[] result = new Physical[childNodes.length];
        for (int i = 0; i < childNodes.length; i++) {
            result[i] = childNodes[i].getValue();
        }
        return result;
    }
    ...
    public void clear() {
        ...
        root = new PhysicalMultiTreeNodeEmpty();
        size = 0;
    }
    ...
    public PhysicalPhysicalMap parentMap() {
        ...
        fillParentMap(root, null, map);
        return map;
    }

    private void fillParentMap(PhysicalMultiTreeNode node, Physical parent, PhysicalPhysicalMap map) {
        if (node.getValue() == null) {
            return;
        }
        if (parent != null) {
            map.put(node.getValue(), parent);
        }
        for (PhysicalMultiTreeNode child : node.getChildren()) {
            fillParentMap(child, node.getValue(), map);
        }
    }
